    ...
    def run(self):
        for f in self.f:
            ...
            for i in range(self.n):
                xi = f.a + i*f.dx
                vol += math.pi * (f.f(xi))**2 * f.dx
                print(f"x_i = {xi} | f(x_i) = {f.f(xi)} | {vol} mL")
            self.volume_each_function.append(vol)

        print(self.volume_each_function)
...
disk = Disk(0, 19.5, 10, [
        Function(lambda x: 1/math.sqrt(3) * math.sqrt(x) + 2.5, 0, 3, 10),
        Function(lambda x: 4/49 * x**2 - 52/49 * x + 583/98, 3, 10, 10),
        Function(lambda x: 3.5, 10, 11.5, 10),
        Function(lambda x: -0.28125*x + 6.734375, 11.5, 19.5, 10)
    ], 350)